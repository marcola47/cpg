...
export async function POST(req: NextRequest) {
  try {
    ...
            })
      ...
      if (!data.genitorId && data.genitorFamilia) {
        const familia = await prisma.familia.upsert({
          where: {
            nome: data.genitorFamilia.trim()
          },
          update: {
            nome: data.genitorFamilia.trim()
          },
      ...
      if (!data.genitoraId && data.genitoraFamilia) {
        const familia = await prisma.familia.upsert({
          where: {
            nome: data.genitoraFamilia.trim()
          },
          update: {
            nome: data.genitoraFamilia.trim()
          },
...
export async function PATCH(req: NextRequest) {
  try {
    //account for no family and remove them from that 'family'
  }
